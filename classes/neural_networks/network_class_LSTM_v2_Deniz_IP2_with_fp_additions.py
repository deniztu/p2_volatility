...
class AC_Network():
    def __init__(self, trainer, noise, coefficient
        ...
        # Input
        self.prev_rewardsch        = tf.placeholder(shape=[None,1], dtype=tf.float32, name = "1")
        self.prev_actions          = tf.placeholder(shape=[None], dtype=tf.int32, name = "2")
        self.prev_actions_onehot   = tf.one_hot(self.prev_actions, n_arms, dtype=tf.float32, name = "3") #changed
        self.timestep              = tf.placeholder(shape=[None,1], dtype=tf.float32, name = "4")
        #self.timestep = tf.constant(1,shape=[1])
        input_                     = tf.concat([self.prev_rewardsch, self.prev_actions_onehot],1)

        self.actions             = tf.placeholder(shape=[None], dtype=tf.int32, name = "5")
        ...
        lstm_cell = tf.nn.rnn_cell.LSTMCell(nb_units,state_is_tuple=True)
        ...
        # Ioannis rollout input
        # shape_of_input = tf.shape(rnn_in) ## the shape of input 
        # step_size = tf.shape(self.prev_rewardsch)[:1] ## the maximum padding shape
                
        shape_of_input = rnn_in.get_shape() ## the shape of input 
        step_size = self.prev_rewardsch.get_shape()[:1] ## the maximum padding shape
                
        
        mask_= tf.sequence_mask(shape_of_input, step_size)
        
        state_in = tf.contrib.rnn.LSTMStateTuple(c_in, h_in)
        
        lstm_outputs, lstm_state= tf.keras.layers.LSTM(lstm_cell, return_sequences=True, return_state=True, name="lstm")(
                        inputs=rnn_in,
                        mask=mask_,
        ...
        # added
        # step_size = tf.constant(1,shape=[1])
        #step_size = 1
        # state_in = tf.contrib.rnn.LSTMStateTuple(c_in, h_in)
        ...
        ## test
        #self.lstm_state = tf_utils.unroll_LSTM(lstm_cell,inputs=rnn_in,initial_state=state_in);
        lstm_c, lstm_h = lstm_state
        
        # added for fixed points analysis
        self.lstm_state = lstm_outputs
...
class Worker():
    def __init__(self, game, trainer, model_path, coefficient, model_name, noise
        ...
        # added for fixed points analysis
        self.lstm_cell = self.ac_network.lstm_cell
        self.lstm_state = self.ac_network.lstm_state
    ...
    def work(self, gamma, sess, saver, train):
        ...
        while True:
            ...
            while d == False:
                ...
                a_dist,rnn_state_new, rnn_true_state_new = sess.run([self.ac_network.policy,self.ac_network.state_out,self.ac_network.true_state_out], 
                                                              feed_dict=feed_dict)
...
class neural_network:
    ...
    def __init__(self 
        ...
        self.model_name = 'fp_lstm_n_{}_p_{}_{}_d_{}_p_{}_a_{}_n_{}_te_{}_id_{}'.format(str(self.noise)[0]
