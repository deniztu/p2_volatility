...
model {
  ...
  for (t in 1:nTrials) {        
    ...
    if (choice[t] != 0) {
      ...
      choice[t] ~ categorical_logit( beta * (v + eb));  // compute action probabilities
...
generated quantities{
  ...
  if (choice[t] != 0) {
    ...
    log_lik[t] = categorical_logit_lpmf(choice[t] | beta * (v + eb));
    predicted_choices[t] = categorical_logit_rng(beta * (v + eb));
