...
file_name <- "lstm2_a2c_nh_%s_lr_0_0001_n_u_p_0_5_ew_0_vw_0_5_dr_0_5_res_d_f_p_0_1_rt_con_a_4_n_300_te_50000_id_%s_test_b_daw_p_%s"
...
pickle_to_df <- function(file_name, id, nh, w){
  
  pickle_file <- sprintf(file_name, nh, id, w)
...
max_df <- pickle_to_df(file_name, id = 1, nh = max(nh_factors), w = 1)
...
for (nh in nh_factors){
  for (id in ids){
    for (w in n_walks){
      ...
      rnn_data <- rbind.fill(rnn_data, pickle_to_df(file_name, id, nh, w))
      # else{
...
save(rnn_data, file = 'rnn_data_nh_factors.Rdata')
