...
class AC_Network():
    def __init__(self, trainer, noise, coefficient
        ...
        self.state_in = (c_in, h_in) # added state_in
        self.rnn_in = tf.expand_dims(input_, [0]) ##unsure about this
        self.step_size = tf.shape(self.prev_rewardsch)[:1]  # added self
        # pdb.set_trace()

        self.state_in = tf.contrib.rnn.LSTMStateTuple(c_in, h_in) # added self
        # added self
        lstm_outputs, lstm_state = tf.nn.dynamic_rnn(lstm_cell, self.rnn_in, initial_state=self.state_in, sequence_length=self.step_size,time_major=False)
        ...
        rnn_out = tf.reshape(lstm_outputs, [-1, nb_units]) # gets rid of batch dimension
...
class Worker():
    ...
    def train(self, rollout, sess, gamma, bootstrap_value):
        ...
        # added grad_
        p_l,e_l,v_n,_, grad_, step_size_e, input_e, state_in_e = sess.run([self.ac_network.policy_loss,
            self.ac_network.entropy,
            self.ac_network.var_norms,
            self.ac_network.apply_grads, 
            self.ac_network.gradient_norm,
            self.ac_network.step_size, 
            self.ac_network.rnn_in, 
            self.ac_network.state_in],
            feed_dict=feed_dict)
        
        print('Shape of the inputs to tf.nn.dynamic_rnn after an episode')
        print('step_size_t')
        print(step_size_e.shape)
        print('input_t')
        print(input_e.shape)
        print('state_in_t')
        print(state_in_e[0].shape)
        print(state_in_e[1].shape)
    ...
    def work(self, gamma, sess, saver, train):
        ...
        while True:
            ...
            while d == False:
                ...
                #                                               feed_dict=feed_dict)
                a_dist,rnn_state_new, rnn_true_state_new, step_size_t, input_t, state_in_t = sess.run([self.ac_network.policy,self.ac_network.state_out,self.ac_network.true_state_out, self.ac_network.step_size, self.ac_network.rnn_in, self.ac_network.state_in], 
                                              feed_dict=feed_dict)
                ...
                print('Shape of the inputs to tf.nn.dynamic_rnn whithin an episode')
                print('step_size_t')
                print(step_size_t.shape)
                print('input_t')
                print(input_t.shape)
                print('state_in_t')
                print(state_in_t[0].shape)
                print(state_in_t[1].shape)
